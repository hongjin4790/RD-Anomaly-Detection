...
# 본 코드는 아래 사이트를 참고하여, 라이브러리 에러를 없애서 만듦 by 이승재
# https://pytorch.org/vision/main/_modules/torchvision/models/convnext.html#ConvNeXt_Tiny_Weights
...
class CNBlock(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        result_keep = result
        #print("result_keep in CNBlock", result_keep.size())
...
class ConvNeXt(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        feature_a = self.features[:2](x)
        feature_b = self.features[:4](x)
        feature_c = self.features[:6](x)

        x = self.features(x)
        x = self.avgpool(x)
        x = self.classifier(x)
        
        return [x, feature_c, feature_b, feature_a]
...
def de_convnext_base(*, weights: Optional[ConvNeXt_Base_Weights] = None, progress: bool = True, **kwargs: Any) -> ConvNeXt:
    ...
    .. autoclass:: torchvision.models.ConvNeXt_Base_Weights
        :members:
    """
    print("ConvNeXt_Base_Weights.verify", ConvNeXt_Base_Weights.verify)
